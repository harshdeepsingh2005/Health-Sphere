...
### Docker Demo
You can run the app with Docker Desktop using:
```bash
...
### CI Demo
This repository now includes a GitHub Actions workflow at [`.github/workflows/ci.yml`](.github/workflows/ci.yml) that runs Django checks and database migrations on every push and pull request.

### CD Demo
When you push to `main`, GitHub Actions will build and publish the Docker image to GitHub Container Registry from [`.github/workflows/docker-publish.yml`](.github/workflows/docker-publish.yml). That is the simplest genuine CD step you can show.
